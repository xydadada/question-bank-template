...
3. 在“解析密钥”页点击 MinerU 按钮，登录官方页面获取自己的 Token。选 MinerU、编号 1，粘贴并保存。需要补充密钥时选更高编号。使用云端题图模型时，同页保存自己的 MiMo Key。
4. 在“模型与知识库”页选择一种 Embedding，点击“下载并配置”。脚本会在官方 WeKnora CLI 中要求登录，下载模型并检查实际维度，随后创建父块、子块和原文三个知识库。选择题图模型并点击应用。本地题图模式还会按需下载本地分类模型。
